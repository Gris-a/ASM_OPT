...
Как видно из графика, результаты оказались плохими, так как в нашем датасете не было слов длины даже больше 30, что даёт самое большее возможное значение хеша в ascii('z') * 30 = 3660, в нашем случае вышло около 2000, но приблизив график была обнаружена возможность равномерного распределения ключей по ячейкам таблицы при достаточно малых размерах таблицы. Поэтому тест был проведён повторно на таблице размера 512 и 1500 словами:
...
| Тест              | Дисперсия |
|:-----------------:|----------:|
| Большая таблица   |   $709.3$ |
| Маленькая таблица |     $2.9$ |
...
Для оптимизации хеш-таблицы таблица заполнялась набором из $150000$ слов, а затем все слова искались в таблице 100 раз. Размер таблицы брался $2^{18}$, так как при таком подходе мы сможем брать остаток от деления с помощью побитового и, а на распределение хеш-функции crc32 это не влияет.
...
| $t_1$            | $t_2$            | $t_3$            | $t_{avg}$              |
|:----------------:|:----------------:|:----------------:|:----------------------:|
| $208 \cdot 10^7$ | $207 \cdot 10^7$ | $208 \cdot 10^7$ | $(208 ± 4) \cdot 10^7$ |
...
Функция, обозначенная как `0x00000000004018c0` это реализованный на ассемблере CRC32, как видно теперь он занимает третью строчку рейтинга. На верхнем месте расположена сама функция поиска. Пока остановим наш взгляд на том что ниже. Нам остается только посмотреть на оптимизации функций `strcmp` и `strlen`. Остановимся на `strcmp`.
...
| $t_1$             | $t_2$             | $t_3$             | $t_{avg}$                 |
|:-----------------:|:-----------------:|:-----------------:|:-------------------------:|
| $97.6 \cdot 10^7$ | $97.7 \cdot 10^7$ | $97.5 \cdot 10^7$ | $(97.6 ± 0.1) \cdot 10^7$ |

### Оптимизация 3 этап

Просмотрев результаты тестирования новой версии таблицы получим следующие результаты:

![img](/readme_assets/prof_opt2.png)

Как видно, единственное что осталось оптимизировать, это сама функция поиска. Единственное что мы можем с ней сделать это переписать на ассемблере, избавившись от недочётов, которые мог бы допустить компилятор при трансляции. Посмотрим, во что функция компилируется через godbolt:

![img](/readme_assets/godbolt_hash_find-O1.png)

Видно, что компилятор выполняет много операций со стеком, также вызов crc32 можно убрать, поместив его исходный код вместо вызова. Таким образом функция перепишется:
...
| $t_1$             | $t_2$             | $t_3$             | $t_{avg}$                 |
|:-----------------:|:-----------------:|:-----------------:|:-------------------------:|
| $85.5 \cdot 10^7$ | $85.5 \cdot 10^7$ | $85.7 \cdot 10^7$ | $(85.5 ± 0.1) \cdot 10^7$ |

### Анализ

Также измерения провелись с оптимизацией компилятора `-O3`.  Общие результаты представлены в таблице:

| Оптимизация | время (-O1)                | ускорение (абсолютное) | ускорение (относительное) | время (-O3)                | ускорение (абсолютное) | ускорение (относительное) |
|:-----------:|---------------------------:|-----------------------:|--------------------------:|---------------------------:|-----------------------:|--------------------------:|
| no opt      | $(834.0 ± 2.0) \cdot 10^7$ | 1.00                   | 1.00                      | $(830.0 ± 3.0) \cdot 10^7$ | 1.00                   | 1.00                      |
| opt  1      | $(208.0 ± 4.0) \cdot 10^7$ | 4.01 ± 0.09            | 4.010 ± 0.090             | $(202.6 ± 0.3) \cdot 10^7$ | 4.10 ± 0.02            | 4.100 ± 0.020             |
| opt  2      | $(97.6  ± 0.1) \cdot 10^7$ | 8.55 ± 0.03            | 2.130 ± 0.040             | $(84.7  ± 0.1) \cdot 10^7$ | 9.80 ± 0.05            | 2.392 ± 0.006             |
| opt  3      | $(85.5  ± 0.1) \cdot 10^7$ | 9.75 ± 0.03            | 1.142 ± 0.002             | $(84.8  ± 0.3) \cdot 10^7$ | 9.79 ± 0.07            | 0.999 ± 0.005             |

Итогом проделанной работы будем считать версию `opt 2` скомпилированную с флагом `-O3`, как самую быструю. Таким образом общее ускорение в своей категории составило около 9.8. Как оказалось, ускорение за счёт переписывания функции на -O3 не дал никакого прироста. чтобы разобраться почему, нужно посмотреть во что функция компилируется:

![img](/readme_assets/godbolt_hash_find-O3.png)

Функция выглядит почти как наша, что говорит о том, что компилятор смог хорошо оптимизировать данную функцию.

Мы использовали для оптимизации ассемблерные инструкции, а как известно языки более высокого уровня разработаны как раз из-за их лучшей читаемости, то есть наши оптимизации затрудняют чтение кода. Оценим самый быстрый полученный вариант программы и изначальный при помощи коэффициента $α = 100 \cdot \frac{T_{old}}{T_{new}} * N^{-1}$, где Ν - количество строк, написанных на ассемблере. В нашем случае α = $69.9 ± 0.4$. Таким образом можно сделать вывод, что ассемблерные оптимизации программы оказались довольно удачными.

## Вывод

Таким образом, можно отметить, что выбор хеш-функции может сильно влиять на скорость работы таблицы, превращая операции, которые могли бы работать за O(1) в O(n), что даёт невероятный прирост скорости при большом размере таблицы. По поводу ассемблерных оптимизаций стоит отметить, что с ними стоит быть аккуратным, так как они могут привязать к архитектуре и усложняют код, но могут дать большой прирост скорости, так как компилятор не может предвидеть все возможные исходы и остаётся педантичным. К тому же он не сможет применить оптимизацию скорости за счёт увеличения объёма потребляемой памяти так, как это было сделано в данной работе. Плохим примером оптимизации в данной работе может служить последняя, переписывание функции на ассемблере, так как компилятор на встроенных оптимизациях смог самостоятельно сгенерировать код не хуже.
