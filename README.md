...
Запускать тесты будем по 3 раза и измерять среднее количество тактов, измеренное при помощи команды `rdtsc`, флаги компилятора при запуске: `-O0 -mavx2`.
...
| $t_1$             | $t_2$             | $t_3$             | $t_{avg}$               |
|:-----------------:|:-----------------:|:-----------------:|:-----------------------:|
| $2077 \cdot 10^7$ | $2073 \cdot 10^7$ | $2071 \cdot 10^7$ | $(2074 ± 2) \cdot 10^7$ |
...
| $t_1$              | $t_2$              | $t_3$              | $t_{avg}$                  |
|:------------------:|:------------------:|:------------------:|:--------------------------:|
| $232.5 \cdot 10^7$ | $233.1 \cdot 10^7$ | $233.3 \cdot 10^7$ | $(232.9 ± 0.3) \cdot 10^7$ |
...
| $t_1$              | $t_2$              | $t_3$              | $t_{avg}$                  |
|:------------------:|:------------------:|:------------------:|:--------------------------:|
| $175.7 \cdot 10^7$ | $177.2 \cdot 10^7$ | $175.4 \cdot 10^7$ | $(176.1 ± 0.7) \cdot 10^7$ |
...
![img](/readme_assets/godbolt_hash_find-O0.png)

Видно, что компилятор выполняет много операций с памятью, вместо того чтобы использовать регистры, также вызовы crc32 и str_compare можно убрать, поместив их исходный код вместо вызова. Таким образом функция перепишется:

![img](/readme_assets/hash_find_asm.png)

Таким образом получившееся время:

| $t_1$             | $t_2$             | $t_3$             | $t_{avg}$                 |
|:-----------------:|:-----------------:|:-----------------:|:-------------------------:|
| $99.0 \cdot 10^7$ | $98.1 \cdot 10^7$ | $99.6 \cdot 10^7$ | $(98.9 ± 0.5) \cdot 10^7$ |

### Анализ

Также измерения провелись с оптимизацией компилятора `-O3`.  Общие результаты представлены в таблице:

| Оптимизация | время (-O0)                 | ускорение (абсолютное) | ускорение (относительное) | время (-O3)                | ускорение (абсолютное) | ускорение (относительное) |
|:-----------:|----------------------------:|-----------------------:|--------------------------:|---------------------------:|-----------------------:|--------------------------:|
| no opt      | $(2074.0 ± 2.0) \cdot 10^7$ | 1.00                   | 1.00                      | $(830.0 ± 3.0) \cdot 10^7$ | 1.00                   | 1.00                      |
| opt  1      | $(232.9  ± 0.3) \cdot 10^7$ | 8.91  ± 0.02           | 8.91 ± 0.02               | $(202.6 ± 0.3) \cdot 10^7$ | 4.10 ± 0.02            | 4.10  ± 0.020             |
| opt  2      | $(176.1  ± 0.7) \cdot 10^7$ | 11.78 ± 0.06           | 1.32 ± 0.01               | $(84.7  ± 0.1) \cdot 10^7$ | 9.80 ± 0.05            | 2.392 ± 0.006             |
| opt  3      | $(98.9   ± 0.5) \cdot 10^7$ | 21.00 ± 0.10           | 1.78 ± 0.02               | $(84.8  ± 0.3) \cdot 10^7$ | 9.79 ± 0.07            | 0.999 ± 0.005             |

Итогом проделанной работы будем считать версию `opt 2` скомпилированную с флагом `-O3`. Таким образом общее ускорение составило около 9.8. Как оказалось, ускорение за счёт переписывания функции на -O3 не дал никакого прироста. чтобы разобраться почему, нужно посмотреть во что функция компилируется:
